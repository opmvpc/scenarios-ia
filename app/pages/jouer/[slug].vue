...
onMounted(() => {
  ...
  // rechargement de page en pleine partie : on reprend directement
  if (route.query.pas !== undefined && partie.existante.value) partie.reprendre(Number(route.query.pas))
  pret.value = true
})
...
function surTouche(ev: KeyboardEvent) {
  if (!partie.enCours.value || ev.altKey || ev.ctrlKey || ev.metaKey) return
...
<template>
  <div :data-encre="scenario.id" class="min-h-dvh">
    ...
    <template v-else>
      <JeuBarre :partie="partie" @fiche="ficheOuverte = true" @regles="reglesOuvertes?.showModal()" @quitter="navigateTo('/')" />
      <div ref="zone">
        <Transition name="ecran" mode="out-in">
          <div :key="cleEcran">
            ...
            <JeuEcranChapitre v-else-if="ecran?.type === 'chapitre'" :partie="partie" :situation="ecran.situation" />
      ...
      <!-- tiroir : la fiche enjeux telle qu'elle doit être sur la table -->
      <Transition name="tiroir">
        <div
          v-if="ficheOuverte"
          class="fixed inset-0 z-40 flex items-end sm:items-center justify-center bg-encre/50 p-0 sm:p-6"
          @click.self="ficheOuverte = false"
          @keydown.esc="ficheOuverte = false"
        >
          <div class="w-full sm:max-w-2xl bg-papier rounded-t-[18px] sm:rounded-[18px] border-[2.5px] border-encre p-5 sm:p-7 max-h-[92dvh] overflow-auto" role="dialog" aria-modal="true" aria-labelledby="titre-fiche">
            <div class="flex items-start justify-between gap-4">
              ...
              <button type="button" class="bouton !min-h-10 !px-4" autofocus @click="ficheOuverte = false">Fermer</button>
            </div>
...
.tiroir-enter-active, .tiroir-leave-active { transition: opacity 180ms ease; }
.tiroir-enter-from, .tiroir-leave-to { opacity: 0; }
</style>
